  ...
  options: [
    {
      User: {
        name: 'user',
        description: 'The user you want to get the suggestions from',
        required: true,
      },
    },
    // {"String": { name: "message-id", description: "If you don't know the user's name or id.", required: false}},
    {
      Integer: {
        name: 'display_amount',
        description: 'The amount of suggestions to display',
        required: false,
      },
    },
  ],
  run: async (client, interaction) => {
    try {
      ...
      if (userInfos != null) {
        ...
        let udata = await db.getAllUserSuggestions(
          interaction.guild.id.toString(),
          userInfos.id.toString()
        ) // only worts with strings idk why
        ...
        if (udata == null || udata === [])
          ...
          let calSugs = await udata
            .slice(Math.max(udata.length - givenDisplayAmount, 0))
            .reverse()
            .map(
              (message, index) =>
                `**${index + 1}.** [${lang.to_suggestion}](<https://discord.com/channels/${interaction.guild.id}/${message.channel_id}/${message.message_id}>)\n\`\`\`\n${message.content}\n\`\`\``
            )
            .join('\n\n')
          ...
          if (sugs) {
            ...
            await interaction.followUp({ content: sugs[0], ephemeral: false })
            for (const sug of sugs) {
              if (sug === sugs[0]) continue
              await interaction.followUp({ content: sug, ephemeral: false })
              await new Promise((resolve) => setTimeout(resolve, 250))
            }
          } else {
            await interaction.followUp({ content: calSugs, ephemeral: false })
          }
        }
      }
