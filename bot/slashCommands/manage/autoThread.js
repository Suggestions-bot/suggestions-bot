  ...
  options: [
    {
      StringChoices: {
        name: 'value',
        description:
          'Automatically create a thread when a suggestion is created?',
        required: true,
        choices: [
          ['Yes', 'true'],
          ['No', 'false'],
  ...
  run: async (client, interaction) => {
    try {
      ...
      try {
        ...
        interaction.reply({
          content: lang.auto_thread_set.replace('{value}', value.toString()),
          ephemeral: true,
        })
      } catch (e) {
        interaction.reply({ content: lang.auto_thread_error, ephemeral: true })
      }
    } catch (e) {
