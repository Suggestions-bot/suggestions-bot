  ...
  options: [
    ...
    },
    {
      StringChoices: {
        name: 'remove',
        description:
          'Remove ALL of the existing suggestion channels? It will only remove the selected or all.',
        required: false,
        choices: [
          ['Yes', 'true'],
          ['No', 'false'],
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (remove === 'true') {
        ...
        try {
          if (channel == null) {
            await db.setServerSuggestionChannels(
              interaction.guild?.id || 0,
              null
            )
            interaction.followUp({
              content: lang.reset_suggestion_channel,
              ephemeral: true,
            })
          } else {
          ...
          interaction.followUp({
            content: lang.reset_suggestion_channel_error,
            ephemeral: true,
          })
        }
      } else {
        ...
        if (channel == null) {
          ...
          if (
            channels?.length >=
            (await db.getServerMaxChannels(interaction.guild?.id || 0))
          ) {
            await interaction.followUp({
              content: lang.max_suggestion_channels,
              ephemeral: true,
            })
            return
          }
          ...
          interaction.followUp({
            content: lang.set_suggestion_channel + ' <#' + channel + '>.',
            ephemeral: true,
          })
        }
        ...
        try {
          ...
          await interaction.followUp({
            content: lang.set_suggestion_channel_error,
            ephemeral: true,
          })
        }
      }
