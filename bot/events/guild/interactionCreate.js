...
async function checkForAutoAccept(interaction, message, lang) {
  ...
  if (accept_upvotes) {
    ...
    const voters = await db.getSuggestionVoters(
        ...
        fields: [
          ...
          embed.fields[1],
          {
            name:
              ...
              `** ${embedData?.accepted_emoji || '<a:accept_bot:1000710815562866759>'}`,
        ...
        fields: [
          ...
          embed.fields[1],
          {
            name:
              ...
              `** ${embedData?.denied_emoji || '<a:deny_bot:1000710816980533278>'}`,
...
async function buttons(interaction) {
  ...
  switch (interaction.customId) {
    case 'up':
      {
        ...
        let voter = await db.getSuggestionVoters(
        ...
        if (
          voter['downvoters'].includes(interaction.user.id) ||
          voter['upvoters'].includes(interaction.user.id)
        ) {
    ...
    case 'down':
      {
        ...
        await db.addSuggestionDownvote(
          message.guild.id.toString(),
          message.id.toString(),
          interaction.user.id.toString()
        )
    ...
    case 'up-rechoice': {
      await interaction.deferUpdate().catch(() => {})
      let message = await interaction.message.channel.messages.fetch(
        interaction.message.reference.messageId
      )
      let embed = message.embeds[0]
      let voters = await db.getSuggestionVoters(
        message.guild.id.toString(),
        message.id.toString()
      )
      voters = voters[0]
      ...
      if (!voters['re_voters'].includes(interaction.user.id)) {
        if (voters['upvoters'].includes(interaction.user.id)) {
          ...
          if (
            await db.addSuggestionDownvoteRevoteUp(
              message.guild.id.toString(),
              message.id.toString(),
              interaction.user.id.toString()
            )
    ...
    case 'down-rechoice': {
      ...
      if (!voters['re_voters'].includes(interaction.user.id)) {
        if (voters['downvoters'].includes(interaction.user.id)) {
          ...
          return
        } else if (voters['upvoters'].includes(interaction.user.id)) {
          if (
            await db.addSuggestionUpvoteRevoteDown(
              message.guild.id.toString(),
              message.id.toString(),
              interaction.user.id.toString()
            )
...
async function modalSubmit(client, modal) {
  if (modal.customId === 'send') {
    ...
    try {
      if (
        serverdata['suggestion_embed_color'] !== null &&
        serverdata['suggestion_embed_color'] !== undefined &&
        serverdata['suggestion_embed_color'] !== ''
      ) {
    ...
    try {
      if (
        serverdata['upvote_emoji'] !== null &&
        serverdata['upvote_emoji'] !== undefined &&
        serverdata['upvote_emoji'] !== ''
      ) {
    ...
    try {
      if (
        serverdata['downvote_emoji'] !== null &&
        serverdata['downvote_emoji'] !== undefined &&
        serverdata['downvote_emoji'] !== ''
      ) {
    ...
    try {
      ...
      console.log(e.stack)
      return await modal.reply({
        content: lang.message_could_not_send_no_perns,
        ephemeral: true,
      })
    }
    ...
    // send the embed
    suggestMessage = await channel.send({
      embeds: [suggestEmbed],
      components: [suggestActionRow],
    })
    ...
    if (udata == null)
      return modal.reply({ content: lang.suggest_none, ephemeral: true })
    let messageIdArray = udata.map((x) => x.message_id)
    if (!messageIdArray.includes(givenMessageID))
      return modal.reply({ content: lang.suggest_none_found, ephemeral: true })
    ...
    if (embed.fields[2] != null) {
      return modal.reply({ content: lang.suggest_cannot_edit, ephemeral: true })
    }
    ...
    modal.reply({ content: lang.suggest_edit_success, ephemeral: true })
  }
}
