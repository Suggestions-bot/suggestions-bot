  ...
  options: [
    {
      Integer: {
        name: 'page',
        description: 'Use no page to reload the cache.',
        required: false,
      },
    },
    {
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (page) {
        ...
        if (page > pageAmount || page < 1) {
          ...
          try {
            ...
            let embed = new discord.EmbedBuilder()
              ...
              .setDescription(
                `Page ${page} of ${cachedGuilds.length} | ${guildAmount} guilds in total | Sorted by memberCount`
              )
              .setColor(0x0000ff)
              .setTimestamp()
              .setFooter({
        ...
        await interaction.deferReply()
        const responseMessage = await interaction.followUp(
          'Getting infos for cached Guilds...\nThis might take a while.'
        )
        const guildAmount = client.guilds.cache.size
        ...
        await responseMessage.edit(
          'Getting suggestion count for each guild...\nETA: <t:' +
            Math.ceil(
              (Date.now() + guildAmount * 110) /*average query time*/ / 1000
            ) +
            ':R>'
        )
        ...
        for (const element of guildArray) {
          element.suggestionCount =
            (await db.getNumberOfSuggestionsInGuild(element.guildId)) || 0
        ...
        if (invites === 'true') {
          const inviteTime =
            guildArray.length * (100 + 350) /*average response time*/
            ...
            if (guildObject.vanityURLCode) {
              ...
              try {
                ...
                let invite = invites.find(
                  (invite) => invite.guild.id === guildId
                )
                if (invite) {
          ...
          await responseMessage.edit(
            'Not getting invites for each guild...\nSorting guilds...'
          )
        }
        ...
        let embed = new discord.EmbedBuilder()
          .setTitle(`Guilds the bot is in (${guildAmount})`)
          .setDescription(
            `Page 1 of ${guildChunks.length} | ${guildAmount} guilds in total | Sorted by memberCount`
          )
          .setColor(0x0000ff)
          .setTimestamp()
          .setFooter({
