...
module.exports = {
  ...
  alloweduserids: [],
  options: [
    { String: { name: 'guild', description: 'Guild ID', required: true } },
  ],
  run: async (client, interaction) => {
    try {
      ...
      if (!guild) {
        ...
        const guildSuggestionCount =
          await db.getNumberOfSuggestionsInGuild(guildId)
        ...
        if (!guildInvite) {
          try {
            guildInvite = await guild.channels.cache
        ...
        const post = execSync(
          `curl -s -d --url https://wastebin.zip/documents --header "content-type: text/plain" --data "${guildObject}"`
        ).toString()
