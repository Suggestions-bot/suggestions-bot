...
async function swap_pages(client, message, description, TITLE) {
  ...
  if (embeds.length === 0)
    return message.channel
      ...
      .catch((e) =>
        Logger.fatal('YAC - Module of YSH prevented following crash:', e)
      )
  if (embeds.length === 1)
    return message.channel
  ...
  let button_back = new MessageButton()
    .setStyle('SUCCESS')
    .setCustomId('1')
    .setEmoji(emojis.sign_arrow_backward)
    .setLabel('Back')
  ...
  let button_forward = new MessageButton()
    .setStyle('SUCCESS')
    .setCustomId('3')
    .setEmoji(emojis.sign_arrow_forward)
    .setLabel('Forward')
  const allbuttons = [
  ...
  collector.on('collect', async (b) => {
    if (b.user.id !== message.author.id)
      return b.reply(
        `${emojis.sign_deny} **Only the one who typed ${prefix}help is allowed to react!**`,
        true
      )
    //page forward
    if (b.customId == '1') {
