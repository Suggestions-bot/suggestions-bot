...
module.exports = (client) => {
  try {
    ...
    readdirSync('./bot/slashCommands/').forEach((dir) => {
      if (lstatSync(`./bot/slashCommands/${dir}`).isDirectory()) {
        ...
        if (cmdSetup && cmdSetup.Folder) {
                ...
                if (pull.options && pull.options.length > 0) {
                  for (const option of pull.options) {
                    if (
                      ...
                      Logger.warn(
                        `A Option is missing the Name and/or the Description of ${pull.name}`
                      )
                    }
                  }
                }
          ...
          return Logger.error(
            `The Subcommand-Folder ${dir} is not in the dirSetup Configuration! You can set it in /src/botconfig/config.json`,
            ''
          )
        }
      } else {
        ...
        if (pull.name && pull.description) {
          let Command = new SlashCommandBuilder()
            .setName(String(pull.name).toLowerCase())
            .setDescription(pull.description) //.setDefaultMemberPermissions(pull.memberpermissions).setDMPermission(pull.dmpermission).setCooldown(pull.cooldown);
          if (pull.options && pull.options.length > 0) {
            for (const option of pull.options) {
    ...
    readdirSync('./bot/guildSlashCommands/').forEach((dir) => {
      if (lstatSync(`./bot/guildSlashCommands/${dir}`).isDirectory()) {
        ...
        if (mainCmdSetup && mainCmdSetup.Folder) {
          ...
          const subCommand = new SlashCommandBuilder()
            .setName(
              String(mainCmdSetup.CmdName).replace(/\s+/g, '_').toLowerCase()
            )
            .setDescription(String(mainCmdSetup.CmdDescription))
            .setDefaultPermission(mainCmdSetup.activeByDefault)
          //Now for each file in that subcommand, add a command!
          const slashCommands = readdirSync(
            `./bot/guildSlashCommands/${dir}/`
          ).filter((file) => file.endsWith('.js'))
          for (let file of slashCommands) {
          ...
          return Logger.error(
            `The Subcommand-Folder ${dir} is not in the serverDirSetup Configuration! You can set it in /src/botconfig/config.json`,
            ''
          )
        }
      } else {
    ...
    client.on('ready', () => {
      if (process.env.STATE === 'production') {
        client.application.commands
          ...
          .then((slashCommandsData) => {
            Logger.info(
              `${slashCommandsData.size} slashCommands (With ${slashCommandsData.map((d) => d.options).flat().length} Subcommands) Loaded.`,
              ''
            )
          })
          ...
          .forEach((guild) => {
            try {
              guild.commands
                ...
                .then((slashCommandsData) => {
                  Logger.info(
                    `${slashCommandsData.size} slashCommands (With ${slashCommandsData.map((d) => d.options).flat().length} Subcommands) Loaded for ${guild.name}.`,
                    ''
                  )
                })
                .catch((e) => Logger.error('An error occured:', e))
      ...
      if (!mainGuild) {
        Logger.error(
          'Could not load the main Server Commands!',
          'The main Server (MAIN_SERVER_ID in .env file) is not in the cache!'
        )
      } else {
        mainGuild.commands
          .set(mainServerOnlyCommands)
          .then((slashCommandsData) => {
            Logger.info(
              `${slashCommandsData.size} slashCommands (With ${slashCommandsData.map((d) => d.options).flat().length} Subcommands) Loaded for main Server "${mainGuild.name}".`,
              ''
            )
          })
          .catch((e) =>
            Logger.error(
              'An error occured whlst loading customGuild commands:',
              e
            )
          )
      }
